...
- `scripts/` — scripted equivalents of every runbook step
  ...
  end-to-end `smoke_test.sh` and the Milestone 2 `provision_worker.py`
  (consumes the CSE Console job queue and runs the whole onboarding).
...
- 🟡 Milestone 2: CSE Console (`cse_console` app) + scripted provisioning
  (`provision_worker.py`) done; remaining: console deployment at
  `console.combatsportseducation.com`, the `last_synced`/`user_count` sync
  job, and the frappe/press fleet setup
